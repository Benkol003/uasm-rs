...
fn build<P>(inc_dir: PathBuf, sources: P, out_dir: &Path)
where
    P: IntoIterator,
    P::Item: AsRef<Path>,
{
    #[cfg(any(target_os = "linux", target_os = "macos"))]
    let plat_def = "__UNIX__";
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    build(inc_dir, sources, &out_dir);

    #[cfg(target_os = "windows")]
    let uasm = out_dir.join("uasm.exe");

    #[cfg(any(target_os = "linux", target_os = "macos"))]
    let uasm = out_dir.join("uasm");

    if !uasm.exists() {
        panic!(
            "expected uasm to be built but did not find {}",
            uasm.display()
        );
    }
